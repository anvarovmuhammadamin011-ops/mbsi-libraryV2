...
function isNavActive(pathname: string, item: NavItem): boolean {
  ...
  return (
    pathname === item.href ||
    (item.href !== "/registrar" && pathname.startsWith(item.href))
  );
}
    ...
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-40 border-b border-border bg-card/95 backdrop-blur-xl">
        <div className="flex items-center justify-between px-4 h-14">
          <Link href="/registrar" className="flex items-center gap-2">
            <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-amber-500/10 text-sm">
              📝
            </span>
      ...
      >
        ...
        <div className="flex h-16 items-center justify-between px-4 border-b border-border">
          {!collapsed && (
            <Link href="/registrar" className="flex items-center gap-2.5">
              <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-amber-500/10 text-base">
                📝
              </span>
              <div className="flex flex-col">
                ...
                <span className="text-[10px] font-medium text-amber-600 leading-none mt-0.5">
                  O&apos;quvchi qo&apos;shuvchi
                </span>
              </div>
            </Link>
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                ...
                >
                  <Icon
                    size={18}
                    className={cn("shrink-0", isActive ? "text-amber-600" : "")}
                  />
        ...
        <div className="border-t border-border p-3">
          <div className="flex items-center gap-3 rounded-xl px-2 py-2">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-amber-500/10 text-sm font-semibold text-amber-600">
              {user.name?.charAt(0)}
            </div>
            {!collapsed && (
              <>
