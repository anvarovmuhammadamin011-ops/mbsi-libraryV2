...
export function ReadingGoalCard() {
  ...
  if (loading) return <div className="rounded-2xl border border-border bg-card p-4 text-sm text-muted-foreground">{t.plan.loading}</div>;
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-4 space-y-4">
      ...
      {goals.length > 0 ? (
            ...
            <div key={g.type} className="rounded-xl bg-muted/30 p-3">
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium flex items-center gap-2">
                  ...
                  {g.type === "BOOKS_PER_MONTH" ? `${g.target} ${t.plan.booksPerMonth}` : `${g.target} ${t.plan.minutesPerDay}`}
                </p>
                <Button size="sm" variant="ghost" className="h-6 w-6 p-0" onClick={() => remove(g.type)} aria-label={t.plan.deleteGoal}>
                  <Trash2 size={12} />
                </Button>
              </div>
              <div className="mt-2">
        ...
        <p className="text-sm text-muted-foreground">{t.plan.noGoals}</p>
      )}
      ...
      <div className="flex gap-2 items-end border-t pt-3">
        ...
        <div className="w-24">
          <label className="text-xs text-muted-foreground">{t.plan.target}</label>
          <Input type="number" min={1} max={100} value={newTarget} onChange={(e) => setNewTarget(Number(e.target.value))} className="mt-1 h-9" />
        </div>
        <Button onClick={create} className="h-9 gap-1"><Plus size={14} /> {t.plan.add}</Button>
      </div>
      <p className="text-[11px] text-muted-foreground/70">{t.plan.example}</p>
    </div>
  );
}
